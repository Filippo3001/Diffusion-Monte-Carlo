...
class MonteCarlo:
    def __init__(self, f, h):
        ...
        self.h = h  # function to find expectation value of

    def integrate(self, r, nMoves, nThermMoves, metroStep):
        ...
        for i in range(nMoves):              #Loop of moves
            ...
            new_h = self.h(r)   #compute the local value for  h
            SE += new_h         #update  accumulation of local values
            SE2 += new_h**2
        
        average = SE/nMoves
        sigma = np.sqrt(SE2/nMoves - (SE/nMoves)**2)
